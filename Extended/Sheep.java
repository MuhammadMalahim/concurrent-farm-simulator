...
public class Sheep extends Animal {
    ...
    protected void move(Farm farm) {
        ...
        for (int[] dir : directions) {
            synchronized (farm.getCell(row + dir[0], col + dir[1])) {
        ...
        Object oldCell = farm.getCell(row, col);
        Object newCell = farm.getCell(newRow, newCol);

        synchronized (oldCell) {
            synchronized (newCell) {
                if (!isValidMove(farm, newRow, newCol)) {
                    return;
                }

                boolean isGateCell = newCell instanceof GateCell;
